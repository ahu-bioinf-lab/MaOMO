...
if __name__ == "__main__":
    ...
    parser.add_argument("--opt", default='obj4',
                        choices=['qed', 'logP', 'qedplogp', 'obj4'])
    ...
    if args.opt == 'qed':
        ...
        ff = ff_obj4
        fitness = fitness_obj4
        col = ['SMILES', 'mol_id','restart', 'qed', 'sim', 'gsk3b','sa_nom']
    ...
    simbank = pd.read_csv('./data/bank library/gsk3_bank.csv').values
    ...
    restart = 1
    adap_tres = np.array([0.7, 0.4, 0.2, 0.7])
    L = 20  # level based dominance
    SR = 0
    #a = list(range(100, 181, 20))  # 180
    #b = list(range(120, 201, 20))  # 200
    a = [0]
    b = [2]
        ...
        for i in range(mm1,mm2):
            ...
            while r < restart:
                ...
                while iter < nIter:
                    ...
                    tres = np.array([0.7, 0.4, 0.2, 0.7])*iter/nIter
                    ...
                    for i in range(len(smis)):
                        if smis[i] not in unique_smiles:
                            ...
                            if (np.array(fits[:][i]) >= np.array([0.7, 0.4, 0.2, 0.7])).all() == 1:
                                rr_pop = rr_pop + 1
                ...
                for i in range(len(endsmiles)):
                    if endsmiles[i] not in unique_smiles:
                        ...
                        tuple = (endsmiles[i], nn,r, endfits[i][0], endfits[i][2], endfits[i][1], endfits[i][3])
                        smi_end_all.append(tuple)
                        if (np.array(endfits[:][i]) >= np.array([0.7, 0.4, 0.2,0.7])).all() == 1:
                            rr.append(1)
